...
object GridPlanner {
    ...
    fun plan(
        ...
        val parts = splitMission(waypoints, settings.maxWaypointsPerMission.coerceIn(20, 200))
        val routeDistance = GeoMath.polylineDistanceM(waypoints)
        // Full automatic mode stops at each waypoint to trigger one photo.
        ...
        return MissionPlan(boundary, waypoints, parts, settings.copy(bearingDeg = bearing), stats)
    }

    /** Pair(width across-track, height along-track), from diagonal FOV and 4:3 image geometry. */
    private fun cameraFootprint(altitudeM: Double, camera: CameraModel): Pair<Double, Double> {
    ...
    private fun findBestBearing(boundary: List<LatLng>, lineSpacing: Double): Double {
        var bestBearing = 0.0
        var bestCost = Double.POSITIVE_INFINITY
        // 0..175 is sufficient because 180° generates the same set of lines.
        var b = 0.0
        while (b < 180.0) {
            val result = generateGrid(boundary, b, lineSpacing, max(4.0, lineSpacing / 2.0), photos = false)
            if (result.waypoints.size >= 2) {
                val cost = GeoMath.polylineDistanceM(result.waypoints) + result.lineCount * 8.0
                if (cost < bestCost) {
                    bestCost = cost
                    bestBearing = b
                }
    ...
    private fun generateGrid(
        ...
        // u follows the desired flight direction; v is cross-track.
        val dX = sin(br)
        val dY = cos(br)
        val nX = cos(br)
        val nY = -sin(br)
        val uv = xy.map { p -> UV(p.x * dX + p.y * dY, p.x * nX + p.y * nY) }

        val minV = uv.minOf { it.v }
        val maxV = uv.maxOf { it.v }
        val span = maxV - minV
        // Keep flight lines inside the polygon instead of placing them exactly on
        // the boundary. This also guarantees one centered line for very narrow areas.
        ...
        for (i in 0 until lineSlots) {
            val v = if (span > 0.01) minV + (i + 0.5) * actualSpacing else minV
            val intersections = mutableListOf<Double>()
            for (j in uv.indices) {
                val a = uv[j]
                val b = uv[(j + 1) % uv.size]
                val crosses = (a.v <= v && b.v > v) || (b.v <= v && a.v > v)
                if (crosses) {
                    val t = (v - a.v) / (b.v - a.v)
            ...
            for (seg in orderedSegments) {
                ...
                val uValues = if (photos) {
                    val photoCount = max(2, ceil(length / photoSpacing).toInt() + 1)
                    (0 until photoCount).map { idx ->
                        val t = if (photoCount <= 1) 0.0 else idx.toDouble() / (photoCount - 1)
                        startU + delta * t
                    }
                } else {
                    listOf(startU, endU)
                ...
                for (u in uValues) {
                    val x = u * dX + v * nX
                    val y = u * dY + v * nY
                    val ll = projection.toLatLng(GeoMath.XY(x, y))
                    if (route.lastOrNull()?.let { GeoMath.distanceM(it, ll) < 0.20 } != true) {
                        route += ll
                    }
                }
                lineNo++
        ...
        return GridResult(route, lineNo)
    }

    private fun splitMission(points: List<LatLng>, maxPerPart: Int): List<List<LatLng>> {
        ...
        while (start < points.size - 1) {
            ...
            start = endExclusive - 1 // shared seam waypoint
        }
        return parts
    }
